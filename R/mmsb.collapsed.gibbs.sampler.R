...
function (network, K, num.iterations, alpha, beta.prior, initial = NULL, 
    burnin = NULL, trace = 0L, thin = NULL) 
{
    ...
    retval <- .pairwise.link.lda.collapsed.gibbs.sampler(empty.docs, 
        ...
        initial, burnin, trace = trace, thin = thin)
    retval <- retval[c("document_sums", "net.assignments.left", "net.assignments.right", 
        "blocks.neg", "blocks.pos", if (is.null(burnin)) NULL else "document_expects",
        if (is.null(thin)) NULL else "samples")]
    if (!is.null(thin))
      names(retval[["samples"]]) <- c("document_sums", "net.assignments.left", 
                                      "net.assignments.right", "blocks.neg", "blocks.pos")
    retval
}
